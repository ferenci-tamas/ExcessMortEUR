...
Egyetlen megjegyzés mielőtt belevágunk: az ilyen monitoring szükségképp ex ante jellegű, értelemszerűen, úgyhogy itt nincs ilyen kérdés. (A programkódokban ugyan az `ExPost` megnevezés van, ha valaki megnézi, de ez ne zavarjon meg senkit: ez a koronavírus-járványra vonatkozik, tehát azért ex post, mert benne van a járvány utáni időszak is -- a járvány utáni időszakban viszont ez az ex ante.)

Nézzük először a Magyarországra vonatkozó eredményeket:

```{r, echo = FALSE}
ggplot(res[nuts_level==0&age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson" & geo == "HU"],
...
```{r, echo = FALSE}
ggplot(res[age=="TOTAL"&(nuts_level==3|geo=="HU")&sens==FALSE&ED=="ExPost"&model=="quasipoisson"],
       aes(x = date, y = excess/population*1e6, group = geoname, label = geoname)) +
  geom_line(aes(color = geo=="HU",
                group = forcats::fct_reorder(geo, geo=="HU", .fun = first))) +
  geom_hline(yintercept = 0, colour = "blue") +
